...
//                $ugly_url = $cleaned_home_url.'?p='.$post->id;
        ...
        $result = $wpdb->get_results($wpdb->prepare("SELECT ID FROM {$wpdb->posts} WHERE (post_content LIKE '%%%s%%' {$redirected} {$ugly_permalink}) {$post_types} {$statuses_query} {$completely_ignore}", $url2));
    ...
    public static function getData($start = 0, $orderby = '', $order = 'DESC', $search='', $limit=20, $orphaned = false)
    {
        ...
        $ignored_terms = '';
        if($hide_ignored){
            $ignored_posts = Wpil_Query::ignoredPostIds();
            if($show_categories){
    ...
    public static function getCustomFieldsInboundLinks($url)
    {
        ...
        $statuses_query = Wpil_Query::postStatuses('p');
        $result = $wpdb->get_results($wpdb->prepare("SELECT m.post_id FROM {$wpdb->postmeta} m INNER JOIN {$wpdb->posts} p ON m.post_id = p.ID WHERE $custom_fields m.meta_value LIKE '%%%s%%' $statuses_query", $url));
