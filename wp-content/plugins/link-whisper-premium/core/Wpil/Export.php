            ...
            switch ($type) {
                case 'links':
                    if(!empty(Wpil_Settings::HasGSCCredentials())){
                        $header = "Title,Type,Category,Published,Organic Traffic,AVG Position,Source Page URL - (The page we are linking from),Outbound Link URL,Outbound Link Anchor,Inbound Link Page Source URL,Inbound Link Anchor\n";
                    }else{
                ...
                case 'links_summary':
                    if(!empty(Wpil_Settings::HasGSCCredentials())){
                        $header = "Title,URL,Type,Category,Published,Organic Traffic,AVG Position,Inbound internal links,Outbound internal links,Outbound external links\n";
                    }else{
    ...
    public static function csv_links_summary($count)
    {
        ...
        foreach ($links['data'] as $link) {
            if (!empty($link['post']->getTitle())) {
                ...
                if($authed){
                    $data .= $title . "," . $url . "," . $type . "," . $category . "," . $date . "," . $post->get_organic_traffic()->clicks . "," . $post->get_organic_traffic()->position . "," . $ii_count . "," . $oi_count . "," . $oe_count . "\n";
                }else{
