...
class Wpil_Model_Post
{
    ...
    function removeUnprocessableBlocks($content){
        ...
        if(in_array('WPSEO_VERSION', $constants) && defined('WPSEO_VERSION') && false !== strpos($content, '<!-- wp:yoast/')){
        ...
        if(in_array('RANK_MATH_VERSION', $constants) && defined('RANK_MATH_VERSION') && false !== strpos($content, '<!-- wp:rank-math/')){
        ...
        if(in_array('WPZOOM_RCB_PLUGIN_FILE', $constants) && defined('WPZOOM_RCB_PLUGIN_FILE') && false !== strpos($content, '<!-- wp:wpzoom-recipe-card/')){
    ...
    function getPostTerms(){
        ...
        $categories = wp_get_object_terms($this->id, $taxes, ['fields' => 'all_with_object_id']);
