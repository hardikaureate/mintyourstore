...
abstract class OrderStatistics {
    ...
    function exportAll($label,$startDate,$endDate,$filter_type,$model) {
    ...
    function exportSingle($startDate,$endDate,$filterId,$dataType,$filter_type,$model) {
        ...
        switch ($dataType) {
            case 'dates': {
                $exportedFile = new CSVWriterFile(
                    array( "Date","Orders",'Gross sales', 'Net sales', 'Total sales')
                );
            ...
            case 'orders': {
                $exportedFile = new CSVWriterFile(
                    array( "Order ID",'Gross sales', 'Net sales', 'Total sales')
                );
            ...
            case 'products': {
                $exportedFile = new CSVWriterFile(
                    array( "Product Name",'Qty', 'Orders', 'Gross sales')
                );
    ...
    function loadStatSingleData() {
        ...
        $chart = [];
        $max = 0;
        // error_log("loadStatSingleData data".print_r($_POST,true));
        switch ($dataType) {
            case 'dates': {
                // error_log("loadStatSingleData dates");
                $data = $this->orderTable->getDatesForSingle($filterColName,$filterId,$startDate,$endDate,$typeId);
                $total = $this->orderTable->getFilterSingleTotal($dataType,$filterColName,$filterId,$typeId,$startDate,$endDate);
                $max = $total[0]["value"];
            }break;
            case 'orders': {
                // error_log("loadStatSingleData orders");
                $data = $this->orderTable->getOrdersForSingle($filterColName,$filterId,$startDate,$endDate,$typeId);
                $total = $this->orderTable->getFilterSingleTotal($dataType,$filterColName,$filterId,$typeId,$startDate,$endDate);
                $max = $total[0]["value"];
            }break;
            case 'products': {
                // error_log("loadStatSingleData products");
